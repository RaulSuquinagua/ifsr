...
var OrdersPage = /** @class */ (function () {
    function OrdersPage(afdb, route, common) {
        this.afdb = afdb;
        this.route = route;
        this.common = common;
        this.orders = [];
    ...
    OrdersPage.prototype.reset = function () {
        this.orders = this.allOrders;
        this.filter = {};
    };
    OrdersPage.prototype.sortByDriver = function () {
        var _this = this;
        var data = this.orders;
        this.orders = data.filter(function (item) {
            if (_this.filter.driverId == 'unassigned') {
                return item.driverId == "";
            }
            else {
                return item.driverId == _this.filter.driverId;
            }
        });
    };
    OrdersPage.prototype.sort = function () {
        var _this = this;
        var data = this.orders;
        this.orders = data.filter(function (item) {
            var createdAt = new Date(item.createdAt).getTime();
            return createdAt >= new Date(_this.filter.from).getTime() && createdAt <= new Date(_this.filter.to).getTime();
        });
    };
    OrdersPage.prototype.sortByStatus = function () {
        var _this = this;
        var data = this.orders;
        console.log(this.filter);
        this.orders = data.filter(function (item) {
            return item.orderStatus == _this.filter.orderStatus;
        });
    };
    OrdersPage.prototype.getAllOrders = function () {
        var _this = this;
        ...
        this.afdb.list('orders/', function (ref) { return ref.orderByChild('createdAt').startAt(startTime).endAt(endTime); }).snapshotChanges().subscribe(function (data) {
            var tmp = [];
            data.forEach(function (order) {
                tmp.push(tslib__WEBPACK_IMPORTED_MODULE_0__["__assign"]({ key: order.key }, order.payload.val()));
            });
            _this.allOrders = tmp;
            _this.orders = tmp.reverse();
            console.log(_this.orders);
        });
    };
    OrdersPage.prototype.getUserOrders = function () {
        var _this = this;
        this.afdb.list('orders', function (ref) { return ref.orderByChild('userId').equalTo(_this.userId); })
    ...
    OrdersPage.prototype.getDriverOrders = function () {
        var _this = this;
        this.afdb.list('orders', function (ref) { return ref.orderByChild('driverId').equalTo(_this.userId); })
            .snapshotChanges().subscribe(function (data) {
            console.log(data);
            var tmp = [];
            data.forEach(function (order) {
    ...
    OrdersPage.prototype.updateStatus = function (key, status) {
        var _this = this;
    ...
    OrdersPage.prototype.updateDriver = function (key, driverId) {
        var _this = this;
        this.afdb.object('orders/' + key).update({
            driverId: driverId
        }).then(function () {
    ...
    OrdersPage.prototype.getDrivers = function () {
        var _this = this;
    ...
    OrdersPage.prototype.view = function (order) {
        this.order = order;
    };
