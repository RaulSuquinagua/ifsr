...
var HomePage = /** @class */ (function () {
    function HomePage(geolocation, afdb, afAuth, backgroundGeolocation, common) {
        var _this = this;
        ...
        this.afAuth.auth.onAuthStateChanged(function (driver) { if (driver != null)
            _this.getDriver(driver.uid); });
    }
    HomePage.prototype.getDriver = function (uid) {
        var _this = this;
        this.common.showLoader();
        this.afdb.object('drivers/' + uid).snapshotChanges().subscribe(function (driver) {
            _this.driver = tslib__WEBPACK_IMPORTED_MODULE_0__["__assign"]({ key: driver.key }, driver.payload.val());
            console.log(_this.driver);
            _this.common.hideLoader();
        });
    };
    HomePage.prototype.fetchOrders = function () {
        var _this = this;
        this.common.showLoader();
        ...
        this.afdb.list('orders/', function (ref) { return ref.orderByChild('createdAt').startAt(start.getTime()).endAt(end.getTime()); }).snapshotChanges().subscribe(function (data) {
            var tmp = [];
            data.forEach(function (order) { return tmp.push(tslib__WEBPACK_IMPORTED_MODULE_0__["__assign"]({ key: order.key }, order.payload.val())); });
            // Filter by drivers
            var driverOrders = tmp.filter(function (o) { return o.driverId == _this.driver.key; });
            _this.orders = driverOrders;
            _this.common.hideLoader();
        });
    };
    HomePage.prototype.changeStatus = function () {
        ...
        if (this.driver.isActive) {
            ...
            this.geolocation.getCurrentPosition({
                ...
                _this.fetchOrders();
                _this.afdb.object('drivers/' + _this.driver.key).update({
                    location: { lat: res.coords.latitude, lng: res.coords.longitude },
                    isActive: true
                });
                _this.location = { lat: res.coords.latitude, lng: res.coords.longitude };
            }).catch(function (err) {
                console.log(err);
            });
            ...
            this.backgroundGeolocation.configure({
                ...
                _this.backgroundGeolocation.on(_ionic_native_background_geolocation_ngx__WEBPACK_IMPORTED_MODULE_5__["BackgroundGeolocationEvents"].location).subscribe(function (location) {
                    ...
                    _this.location = { lat: location.latitude, lng: location.longitude };
                    _this.afdb.object('drivers/' + _this.driver.key).update({
